...
class LmsController extends Controller
    ...
    public function lms( Request $request, $slug = '' )
 {
        ...
        $category = LmsCategory::getRecordWithSlug( $slug );

        if ( $category ) {
            $records             = LmsSeries::where( 'lms_category_id', '=', $category->id )
            ...
            foreach ( $records  as $item ) {
                ...
                $temp[ 'is_purchased' ] = ( int )Payment::isItemPurchased( $item->id, 'lms', $user->id );
                ...
                if ( !isItemPurchased( $record->id, 'lms' ) )
 {
        ...
        $response[ 'is_purchased' ]   = ( int )Payment::isItemPurchased( $record->id, 'lms', $user->id );
        ...
        if ( $user->settings )
 {
        ...
        if ( $interested_categories ) {
            ...
            if ( count( $interested_categories->lms_categories ) ) {
                ...
                $categories  = LmsCategory::whereIn( 'id', ( array ) $interested_categories->lms_categories )
                ->pluck( 'id' )
                ->toArray();
        ...
        if ( count( $categories ) > 0 ) {
            ...
            $recs = LmsSeries::whereIn( 'lms_category_id', $categories )->get();
            ...
            foreach ( $recs as $r )
 {
                ...
                $total_items  = LmsSeriesData::where( 'lmsseries_id', $r->id )->get()->count();
                $temp[ 'title' ]              = $r->title;
                $temp[ 'id' ]                 = $r->id;
                $temp[ 'slug' ]               = $r->slug;
                ...
                $temp[ 'is_purchased' ]       = ( int )Payment::isItemPurchased( $r->id, 'lms', $user->id );
