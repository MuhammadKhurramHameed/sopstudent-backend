...
class GeneralController extends Controller
    ...
    public function getTopExamAndLms( Request $request )
 {
        ...
        $exam_records = Quiz::select( [ 'id', 'title', 'slug', 'dueration', 'category_id',
        ...
        $lms_series =   LmsSeries::select( [ 'id', 'title', 'slug', 'is_paid', 'cost',
        ...
        foreach ( $exam_records as $item )
 {
            ...
            $temp[ 'is_purchased' ] = ( int )Payment::isItemPurchased( $item->id, 'combo', $user->id );
 ...
 {
            $examfinish = DB::table('examfinish')
            ->select('examfinish_id')
            ->where('exam_slug','=',$item->slug)
        ...
        if ( $user->settings )
 {
        ...
        if ( $interested_categories ) {
            ...
            $response[ 'categories' ]         = QuizCategory::
        ...
        if ( $user->settings )
 {
        ...
        if ( $interested_categories ) {
            ...
            $response[ 'categories' ]       = LmsCategory::
    ...
    public function notifications()
 {
        ...
        $response[ 'message' ] = '';
        $date = date( 'Y-m-d' );
        $records = Notification::where( 'valid_from', '<=', $date )
    ...
    public function validateCoupon( Request $request )
 {
        ...
        if ( $item_type == 'combo' )
 {
            $item = ExamSeries::where( 'slug', '=', $request->item_name )->first();
        } else if ( $item_type == 'exam' ) {
            $item = Quiz::where( 'slug', '=', $request->item_name )->first();
        } else if ( $item_type == 'lms' ) {
            $item = LmsSeries::where( 'slug', '=', $request->item_name )->first();
        }
        ...
        $coupon_data = Couponcode::where( 'coupon_code', '=', $coupon_code )->first();
        if ( $coupon_data ) {
    ...
    public function getPaymentGateways()
 {
    ...
    public function listenerUrl(){
        return ('alpapPaylistener');
    }
