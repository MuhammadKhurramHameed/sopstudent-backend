...
class ExamsController extends Controller
        ...
        foreach ( $records as $item )
 {
            ...
            $temp[ 'is_purchased' ] = ( int )Payment::isItemPurchased( $item->id, 'exam', $user->id );
        ...
        if ( !$record )
 {
        ...
        $temp[ 'end_date' ]       = $r->end_date;
        $temp[ 'is_purchased' ] = ( int )Payment::isItemPurchased( $r->id, 'combo', $user->id );
        ...
        if ( $user->settings )
 {
        ...
        if ( $interested_categories ) {
            ...
            if ( count( $interested_categories->quiz_categories ) ) {
                ...
                $categories   = QuizCategory::whereIn( 'id', ( array ) $interested_categories->quiz_categories )
                ->pluck( 'id' )
                ->toArray();
            }
        ...
        if ( count( $categories ) > 0 ) {
            ...
            $recs    = ExamSeries::whereIn( 'category_id', $categories )->get();
            $records = [];
    ...
    public function finishExam( Request $request, $slug )
 {
        ...
        $user_record = User::where( 'id', $formated_data[ 'student_id' ] )->first();
        $user = $user_record;
        $response[ 'status' ] = 0;
        $response[ 'message' ] = '';
        if ( !$quiz )
        ...
        foreach ( $questions as $q )
 {
        ...
        try {
            ...
            $user_record->notify( new StudentExamResult( $user_record, $exam_status, $quiz->title, $quiz->pass_percentage ) );
    ...
    public function getExamKey( $slug )
 {
        ...
        $quiz_result         = QuizResult::where( 'slug', $slug )->first();

        $quiz                = Quiz::where( 'id', $quiz_result->quiz_id )->first();
        $questions = [];
        $data[ 'status' ] = 0;
        $sno = 1;
        if ( !$quiz )
